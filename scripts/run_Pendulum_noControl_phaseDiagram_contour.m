...
fig1 = figure();
    ...
    set(fig1, 'Position', [fig1.Position(1:2) width*100, height*100]); %<- Set size
subplot(121) 
    thetC = linspace(-pi*1.5,pi*1.5,51);
    thetDC =  linspace(-pi*1.5,pi*1.5,51);
    [X,Y] = meshgrid(thetC,thetDC); 
    E = mp*g*(cos(X)-1)*L + 0.5*mp*(L*Y).^2;
    
%     hold off
    
    surf(X,Y,E-max(E(:)),'FaceAlpha',0.5 )
    ax = gca();
    set(ax,'CLim',[-max(E(:)) ,0])
    hold on
    % contour(X,Y,E,10,'Color',[1,1,1]*greyVal)
    contour(X,Y,E,[0:10:50 60:20:200],'Color',[1,1,1]*greyVal)
    colormap(cMapInterp)
    
    
%     [U,V] = gradient(E); 
%     quiver(X(1:end-1)',Y(1:end-1)',Y(1:end-1)',diff(Y(:)))
...
subplot(122) 
    ...
    thetC = linspace(-0.5,0.5,51) +pi;
    thetDC =  linspace(-0.5,0.5,51);
    [X,Y] = meshgrid(thetC,thetDC); 
    E = mp*g*(cos(X)-1)*L + 0.5*mp*(L*Y).^2;
    ...
    surf(X,Y,E-max(E(:)),'FaceAlpha',0.5 )
    ax2 = gca();
    set(ax2,'CLim',[-5,0],'XLim',[-1,1]*0.3+pi,'YLim',[-1,1]*0.3)
    hold on
    contour(X,Y,E,20,'Color',[1,1,1]*greyVal)
%     contour(X,Y,E,[0:10:50 60:20:200],'Color',[1,1,1]*greyVal)
    colormap(cMapInterp)
